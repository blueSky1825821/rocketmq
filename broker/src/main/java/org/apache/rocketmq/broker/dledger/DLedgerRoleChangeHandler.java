                ...
                try {
                    ...
                    switch (role) {
                        ...
                        case CANDIDATE:
                            ...
                            break;
                            //当前节点状态机状态为FOLLOWER，broker节点将转为从节点
                        case FOLLOWER:
                            brokerController.changeToSlave(dLedgerCommitLog.getId());
                            break;
                            //节点状态机状态为Leader，说明该节点被选举为Leader，在切换到Master节点前，首先需要等待当前节点追加的数据都已经被提交后才可以将状态变更为Master
                        case LEADER:
                            //循环等待，在切换到 Master 节点之前，首先需要等待当前节点追加的数据都已经被提交后才可以将状态变更为 Master
                            while (true) {
